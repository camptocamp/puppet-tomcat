...
#  }
  ...
  $file_suffix = regsubst($title, '\/', '-', 'G')
  file {"${_basedir}/${instance}/conf/resourcelink-${file_suffix}.xml":
    ensure  => $ensure,
    owner   => $owner,
    group   => $group,
    mode    => $filemode,
    content => template('tomcat/resourcelink.xml.erb'),
    replace => $manage,
#    require => $require,
  }
  file {"${_basedir}/${instance}/conf/resource-${file_suffix}.xml":
    ensure  => $ensure,
    owner   => $owner,
    group   => $group,
    mode    => $filemode,
